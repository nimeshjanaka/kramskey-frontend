...
export default function AddBreakdown() {
  ...
  const handleImageSelect = (e) => {
    ...
    if (files.length > remaining) {
      setError(`Maximum 5 images allowed. Added ${toAdd.length} image(s).`);
    }
  };
  ...
  const handleSubmit = async () => {
    if (!note.trim()) {
      setError('Please enter a breakdown note');
      return;
    }
  ...
  return (
    <>
      ...
      <div className="page">
        ...
        <button className="btn-secondary" onClick={() => navigate(`/machine/${id}`)}>
          CANCEL
        </button>
      </div>
    </>
